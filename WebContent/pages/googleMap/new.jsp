...
<script type="text/javascript">
	...
	function getOptions() {
		...
		if (isYear && !isMonth && !isDate && !isLocation) {
			...
			if (year && month) {
				...
				+ '&lng=' + lng + '&diameter=' + diameter +'&year=' + year;
			} else {
				alert("Please Choose Year!");
			}
		}else if ((!isYear && isMonth && !isDate && isLocation)){
			...
			if (year && month) {
				...
				sUrl = 'speciesSpotRecordJsonByYearMonthLocation.action?lat=' + lat
				+ '&lng=' + lng + '&diameter=' + diameter +'&year=' + year
						+ '&startMonth=' + startMonth + '&endMonth=' + endMonth;
			} else {
				alert("Please Choose Year & Month!");
			...
			if (year && month) {
				...
				sUrl = 'speciesSpotRecordJsonByPeriodLocation.action?lat=' + lat
				+ '&lng=' + lng + '&diameter=' + diameter +'&startDate='
				+ startDate + '&endDate=' + endDate;
			} else {
				alert("Please Input Date!");
			}
		...
		if (bXmlHttpSupport) {
			oRequest.onreadystatechange = function() {
				...
				//		markerList.push(marker);
			          ...
			          var markers = [];
			          for (var i = 0; i < 10; ++i) {
			            var latlng = new google.maps.LatLng(40, -100);//data.photos[i].latitude, data.photos[i].longitude
	...
	alert(nameChoice);
		popitup("mapResPass.jsp?options="+options+"&"+sUrl.substring(sUrl.indexOf('?')+1)+"&nameChoice="+nameChoice);
...
<div style="width: 20%; float: left">
	...
	<p>
		<input type="checkbox" name="byYear">By Year</input>
	</p>
	<p>
		<input type="checkbox" name="byMonth">By Month</input>
	</p>
	<p>
		<input type="checkbox" name="byDate">By Date</input>
	</p>
	<p>
		<input type="checkbox" name="byLocation" id="byLocation" onClick="drawCircle()">By Location</input>   
	</p>

	<p>
		<s:url id="yearList" action="autoYears.action" />
		<sx:autocompleter label="Year" href="%{yearList}" name="year" />
	</p>
	<p>
		<label for="Month">Month range:</label> <input type="text"
			name="Month" id="Month"
			style="border: 0; color: #f6931f; font-weight: bold;" />
	</p>
	<div id="slider-monthrange"></div>
	<p>
		Start Date: <input type="text" name="startDate" id="startDatepicker" />
	</p>
	<p>
	...
	<input type="submit" id="submit" name="submit" value="Show Points"
		onclick="drawpoints()" /> 
	<input type="submit" id="clear" name="clear" value="Clear All Points"
		onclick="clearAllpoints()" /> 
	<input type="submit" id="submit" name="submit" value="Show Species Tree"
		onclick="showSpeciesTree()" />
	<s:radio label="Output Name" name="nameChoice" id="nameChoice" list="#{'1':'Scientific Name','2':'Common Name'}" value="1" /> 
