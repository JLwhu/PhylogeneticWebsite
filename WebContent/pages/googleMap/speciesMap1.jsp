...
<script type="text/javascript">
	...
	function getOptions() {
		...
		var speciesid = document.getElementsByName("name")[0].value;
		...
		if ($('input:checkbox[name=bySpecies]').is(':checked')) {
			isSpecies = true;
		}

		if (isYear && !isMonth && !isDate && !isLocation && !isSpecies) {
			if (year) {
				options = "ByYear";
		...
		} else if (!isYear && isMonth && !isDate && !isLocation  && !isSpecies) {
			if (month) {
				options = "ByMonth";
		...
		} else if (isSpecies && !isYear && !isMonth && !isDate && !isLocation) {
			if (speciesid) {
				options = "BySpeciesId";
				sUrl = 'speciesSpotRecordJsonBySpeciesid?name='
						+ speciesid;
			} else {
				alert("Please Input Species!");
			}
		}
	}
		...
		//			alert(oSpecies.speciesRecord.length);
					for ( var i = 0; i < oSpecies.speciesRecord.length; i++) {
						...
						var marker = new google.maps.Marker({
							...
							map : map,
							icon : image//,
				//			title : 'Drag me!'// oSpecies.speciesRecord[i].scientificName
						});
...
<div style="width: 20%; float: left">
	...
	<p>
		<input type="checkbox" name="bySpecies">By Species</input>
	</p>
	<p>
	...
	<p>
		<s:url id="nameList" action="autoNames.action" />
		<sx:autocompleter label="Species Names" href="%{nameList}" name="name" />
	</p>

	<input type="submit" id="submit" name="submit" value="Show Points"
		onclick="drawpoints()" /> <input type="submit" id="clear"
		name="clear" value="Clear All Points" onclick="clearAllpoints()" /> <input
		type="submit" id="submit" name="submit" value="Show Species Tree"
